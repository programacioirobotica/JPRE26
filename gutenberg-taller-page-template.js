(function () {
  const root = document.getElementById("jpre-taller-page");
  if (!root) return;
  ...
  const webAppUrl = root.dataset.webappUrl || defaultWebAppUrl;
  ...
  function jsonp(url, params) {
    return new Promise((resolve, reject) => {
      ...
      const callbackName = `jpreTallerJsonp_${nonce}`;
  ...
  function findTaller(allTallers) {
    ...
    const queryNum = String(q.get("num") || "").trim();
    const queryId = String(q.get("taller") || "").trim();
    const queryNom = String(q.get("nom") || "").trim();
    const queryFranja = String(q.get("franja") || "").trim();

    const finalNum = targetNum || queryNum;
    if (finalNum) {
      const byNum = allTallers.find((t) => String(t.num || "").trim() === finalNum);
      if (byNum) return byNum;
    }

    const finalId = targetId || queryId;
    if (finalId) {
      const byId = allTallers.find((t) => String(t.id || "").trim() === finalId);
      if (byId) return byId;
    }

    const finalNom = targetNom || queryNom;
    const finalFranja = targetFranja || queryFranja;
    if (!finalNom) return null;

    const nomN = normalize(finalNom);
    const franjaN = normalize(finalFranja);
  ...
  function render(taller) {
    ...
    const ratio = placesTotals > 0 ? Math.max(0, Math.min(1, placesDisponibles / placesTotals)) : 0;
  ...
  async function init() {
    try {
      ...
      if (!data || !Array.isArray(data.franges)) {
        showError("No s'ha rebut una resposta vàlida del servidor.");
        return;
      }
      ...
      if (!taller) {
        showError("No s'ha trobat aquest taller. Revisa data-num.");
        return;
      }
      render(taller);
    } catch (error) {
  ...
  function boot() {
    const rootEl = document.getElementById("jpre-taller-page");
    if (!rootEl || rootEl.dataset.jpreInitDone === "1") return;
    rootEl.dataset.jpreInitDone = "1";
    init();
  }
